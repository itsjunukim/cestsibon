...
export function NoticePopup() {
    ...
    useEffect(() => {
        ...
        const hideNoticePermanently = localStorage.getItem("hideUpdateNotice_202605_v4")
        const hideNoticeThisSession = sessionStorage.getItem("hideUpdateNoticeSession_202605_v4")
    ...
    const handleClose = () => {
        if (doNotShow) {
            localStorage.setItem("hideUpdateNotice_202605_v4", "true")
        } else {
            // 세션 스토리지에 저장하여, 탭을 닫거나 재로그인하기 전까지는 다시 안 뜨게 함
            sessionStorage.setItem("hideUpdateNoticeSession_202605_v4", "true")
        }
        setOpen(false)
    }
            ...
            <DialogContent className="sm:max-w-md bg-white border-slate-200 max-h-[80vh] overflow-y-auto">
                ...
                <div className="space-y-3 py-4 text-sm text-slate-700">
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                        <h4 className="font-bold text-sm mb-1 text-slate-800">1. 내일 예약 현황 간편 공유 기능</h4>
                        <p className="leading-relaxed text-slate-600 text-xs">예약 관리 상단에 <b>[공유] 버튼</b>이 추가되었습니다. 내일 일정을 단체 톡방 등에 쉽게 복사/공유할 수 있습니다.</p>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                        <h4 className="font-bold text-sm mb-1 text-slate-800">2. 예약 결제 상태 표시 개선</h4>
                        <p className="leading-relaxed text-slate-600 text-xs">잔금 결제 수단이 확정되면, 예약이 아직 완료되지 않았더라도 금액 표기가 <b>초록색</b>으로 표시되도록 개선되었습니다.</p>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                        <h4 className="font-bold text-sm mb-1 text-slate-800">3. 정산 관리 내 예약 상세조회 연동</h4>
                        <p className="leading-relaxed text-slate-600 text-xs">정산 관리 메뉴에서 <b>예약건 이름</b>을 클릭하면 예약 상세 화면이 팝업으로 열려, 즉시 조회 및 수정이 가능합니다.</p>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                        <h4 className="font-bold text-sm mb-1 text-slate-800">4. 예약 폼(수정) 화면 전면 개편</h4>
                    ...
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                        <h4 className="font-bold text-sm mb-1 text-slate-800">5. 맞춤 알림 기능</h4>
                        <p className="leading-relaxed text-slate-600 text-xs">예약 수정 화면에서 <b>특정 시점에 표시할 알림</b>을 직접 등록할 수 있습니다.</p>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                        <h4 className="font-bold text-sm mb-1 text-slate-800">6. 예약 유형 선택 UI 개선</h4>
                        <p className="leading-relaxed text-slate-600 text-xs">새 예약 시 <b>당일 / 숙박</b>을 먼저 선택하면 유형에 맞는 입력 폼이 표시됩니다.</p>
                    </div>
                    <div className="bg-slate-50 p-3 rounded-lg border border-slate-100">
                        <h4 className="font-bold text-sm mb-1 text-slate-800">7. 직원 계정 권한 변경</h4>
